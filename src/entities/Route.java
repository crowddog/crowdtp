...
public class Route {
	...
	public Route(List<String> newRoute, double newscore, double newsatScore,
		...
		calPotentialScore(Constants.userRequireSceneNum - getLengthOfRoute(),
	...
	public void calPotentialScore(int counter,
		...
		for (int i = 0; i < availableScene_start.size(); i++) {
			...
			double temp_satScore = lastRouteSatScore * Constants.maxVoteRatio;
	...
	public Route addScene(String sceneName, double impSatRatio) {
		...
		double newtraScore = scene.getTrafficInfo(scene.getSceneIdByName(route
				...
				/ Constants.transRatio + traScore;
		double newscore = newsatScore - newtraScore;
		Route newRoute = new Route(newroute, newscore, newsatScore,
