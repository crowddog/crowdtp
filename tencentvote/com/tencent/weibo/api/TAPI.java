...
public class TAPI extends BasicAPI{
    ...
    //我的封装类
    private String voteCreateUrl = apiBaseUrl+"/vote/createvote";
    private String voteGetStatisticsInfoUrl = apiBaseUrl + "/vote/get_statistics_info";
    ...
    public void setAPIBaseUrl(String apiBaseUrl) {
        ...
        //我的封装类
        voteCreateUrl = apiBaseUrl+"/vote/createvote";
        voteGetStatisticsInfoUrl = apiBaseUrl + "/vote/get_statistics_info";
    }
	 ...
	 * @param title	 投票标题
	 * @param desc 投票简介（可选）
	 * @param choicetype 投票类型,1-文字类型,2-图片类型
	 * @param choicetexts “|”分隔,最多33个,最少2个,与微博主站一致
	 * @param choicepics “|”分隔,个数与choices一致,图片url顺序与choices中投票选项顺序相对应,图片url为微博域名下url,可通过使用upload_pic接口先上传图片,再将获取到的图片url作为此参数的值（choicetype=1时不用填写）
     * @param limit  投票选项个数限制,1-单选,n表示最多选n项,其中n<=投票选项个数，默认为1，即单选
     * @param status 0-结果所有人可见,1-结果投票后可见，默认为0
     * @param endtime 投票中止时间，用UNIX时间表示（从1970年1月1日0时0分0秒起至现在的总秒数），默认为2147483647，即2038-01-19 11:14:07
	 * @return
	 * @throws Exception
     * @see <a href="http://wiki.open.t.qq.com/index.php/%E5%BE%AE%E5%8D%9A%E7%9B%B8%E5%85%B3/%E5%8F%91%E8%A1%A8%E8%A7%86%E9%A2%91%E5%BE%AE%E5%8D%9A">腾讯微博开放平台上关于此条API的文档</a>
	 */
	...
	public String createVote(
			OAuth oAuth, String format, String title,
			String desc, String picurl, String choicetype, String choicetexts, String choicepics,
			String limit, String status, String endtime) throws Exception {
		QArrayList paramsList = new QArrayList();
		paramsList.add(new BasicNameValuePair("format", format));
		paramsList.add(new BasicNameValuePair("title", title));
		paramsList.add(new BasicNameValuePair("desc", desc));
		paramsList.add(new BasicNameValuePair("picurl", picurl));
		paramsList.add(new BasicNameValuePair("choicetype", choicetype));
		paramsList.add(new BasicNameValuePair("choicetexts", choicetexts));
		paramsList.add(new BasicNameValuePair("choicepics", choicepics));
		...
		return requestAPI.postContent(voteCreateUrl, paramsList,
				oAuth);
	}
    ...
    public String getStatisticsInfo(OAuth oAuth, String format, String name,
    		String voteid) throws Exception{
    	QArrayList paramsList = new QArrayList();
    	paramsList.add(new BasicNameValuePair("format",format));
    	paramsList.add(new BasicNameValuePair("name",name));
    	paramsList.add(new BasicNameValuePair("voteid",voteid));
    	
    	return requestAPI.postContent(voteGetStatisticsInfoUrl, paramsList, oAuth);
...
//    public String getNewAccessToken(OAuth oAuth)throws Exception
//    {
//    	QArrayList paramsList = new QArrayList();
//    	paramsList.add(new BasicNameValuePair("client_id","801435310"));
//    	paramsList.add(new BasicNameValuePair("grant_type","refresh_token"));
//    	paramsList.add(new BasicNameValuePair("refresh_token","c66b04c6d8c2e94210d93836be6d998c"));
//    	String result=requestAPI.postContent("https://open.t.qq.com/cgi-bin/oauth2/access_token", paramsList, oAuth);
